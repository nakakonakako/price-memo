    ...
    <section className="space-y-8 text-stone-800 lg:space-y-10">
      ...
      <div className="space-y-3">
        ...
        <ul className="list-inside list-disc space-y-2 text-base text-stone-700 lg:text-lg">
          ...
          <li>候補から選ぶか「追加」で新規作成。一致しなければ新規作成できます。追加した品目は一覧の一番上に来ます</li>
          <li>カードをタップして開き、確認日・店名・値段（総額）・内容量を入力すると試算単価が表示されます</li>
          <li>「統計に残す」でフォルダの記録として保存（店名は保存時に必須）</li>
          <li>単位は g / ml / 個 のほか「その他…」で自由入力（内容量の下に欄が出ます。入力確定は欄から指を離したとき）</li>
          <li>保存するとフォルダ側の記録にも残り、平均・最安・直近の統計が更新されます</li>
          <li>店名を入れると、その店の記録だけに絞った統計を表示できます</li>
          {isMobile ? (
            <>
              <li>閉じたカードはタップで入力フォームを開きます</li>
              <li>
                メモから外す: カードを<strong>左へ十分スワイプ</strong>すると削除。途中で止めると元に戻ります（削除ボタンを出して止める操作はありません）
              </li>
              <li>カードを開いた状態でも、入力欄以外の余白などから同様に左スワイプでメモから外せます</li>
            </>
          ) : (
            <>
              <li>閉じたカードをタップして入力フォームを開きます</li>
              <li>ドラッグで並べ替え、右側の削除エリアへドロップでメモから外せます（削除エリアはドラッグ中のみ表示）</li>
            </>
          )}
          <li>長くスクロールしたときは右下の ↑ でページ上部へ戻れます</li>
        </ul>
      </div>
      ...
      <div className="space-y-3">
        ...
        <ul className="list-inside list-disc space-y-2 text-base text-stone-700 lg:text-lg">
          <li>
            <strong>品目名</strong> … 鶏むね・牛乳などのフォルダ。カードをタップすると直近 3
            件の記録がプレビューされます（短い日付・総額／内容量。単価の円/g 表示は出しません）
          </li>
          <li>
            <strong>店名</strong> … よく行く店舗の一覧。同様にプレビューがあります
          </li>
          <li>
            すべての記録を見るときは、プレビュー下のボタンか、件数バッジから詳細一覧へ進みます（購入日の新しい順）
          </li>
          <li>詳細一覧では複製・編集ができ、並べ替えはありません</li>
          <li>虫眼鏡でフォルダ名・店舗名をまとめて検索（候補は両方のセクションに出ます。検索中は新規追加カードは出ません）</li>
          {isMobile ? (
            <>
              <li>品目・店名カード、フォルダ内の記録は左スワイプで削除（記録ありのフォルダ削除は確認あり）</li>
              <li>記録行の右端に複製・編集ボタンがあります</li>
            </>
          ) : (
            <>
              <li>品目・店名・記録の削除は、ドラッグ中だけ右側に出るゴミ箱へドロップ</li>
              <li>グラフボタンで値段推移パネルを横に表示</li>
            </>
          )}
          <li>長くスクロールしたときは右下の ↑ でページ上部へ戻れます</li>
        </ul>
      ...
      <div className="space-y-3">
        ...
        <ul className="list-inside list-disc space-y-2 text-base text-stone-700 lg:text-lg">
          ...
          <li>
            <strong>単位</strong> … プリセット以外は「その他…」→ 内容量の下の欄に入力。1文字ずつ確定されず、欄からフォーカスが外れたときに確定
          </li>
          <li>
